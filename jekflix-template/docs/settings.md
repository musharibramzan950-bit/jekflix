...
```yaml
# site.yml
description: Jekflix is a template for Jekyll inspired by Netflix and made by Musharib.
```
...
The social settings can be found in `src/yml/social.yml`. Social media icons appear in the footer of every page.
...
The theme settings are in `src/yml/theme.yml`. You just need to change this file and run `gulp build`.

> **IMPORTANT:** Only when cloning the repo. If you're using the `gem`, follow the README instructions.
...
Set the base hostname and protocol for your site, e.g. `https://example.com`
